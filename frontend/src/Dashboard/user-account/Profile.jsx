...
const Profile = ({ user }) => {
  ...
  useEffect(() => {
    setFormData({
      ...
      bloodType: user.bloodType,
      age: user.age || "",
      previousDiseases: user.previousDiseases || "",
  ...
  const handleReportFileInputChange = async (event) => {
    const file = event.target.files[0];
    const data = await uploadImageToCloudinary(file);
    setSelectedReportFile(file);
    setFormData({ ...formData, report: data.url });
  };
    ...
    <div className="mt-10">
      <form onSubmit={submitHandler}>
        ...
        <div className="mb-5 flex items-center gap-3">
          {formData.report && (
            <figure className="w-[60px] h-[60px] rounded-full border-2 border-solid border-primaryColor flex items-center justify-center">
              <img src={formData.report} alt="Report" className="w-full rounded-full" />
            </figure>
          )}
          ...
          <div className="relative w-[130px] h-[50px]">
            ...
            <label
              htmlFor="customReportFile"
              className="absolute top-0 left-0 w-full h-full flex items-center px-[0.75rem] py-[0.375rem] text-[15px] leading-6 overflow-hidden bg-[#0066ff46] text-headingColor font-semibold rounded-lg truncate cursor-pointer"
            >
              {selectedReportFile ? selectedReportFile.name : "Upload Report"}
            </label>
          </div>
        </div>
